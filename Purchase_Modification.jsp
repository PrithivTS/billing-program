...
input {
	...
	<%
		...
		int itemrate[] = new int[20];
			...
			items = Integer.parseInt(splitted[4]);
			name = "";
			int impt = splitsize - 5 * items - 2;
			for(int j=5; j < impt; j++)
				name += splitted[j] + " ";
			...
			for(int j=1; j<=items;j++)
			{
				...
				itemrate[j-1] = (int)Math.round(Double.parseDouble(splitted[impt+(j-1)*5+6]));
			}
	...
	<h4>PURCHASE INVOICE DETAILS FOR INVOICE NO. <%=billno%></h4>
	</center>
    ...
    document.getElementById('content').appendChild(div);
  	...
  	function startSearch()
  	{
		...
		for(i=1; i<counter;i++)
		{
  		...
  		document.frm.action="Purchase_Finalmodification.jsp?counter="+(counter-1);
  		document.frm.submit();
	}
